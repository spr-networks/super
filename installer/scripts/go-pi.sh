...
# disable iface renaming on the boot image and bump CMA for ath12k
...
if ! grep -q "cma=128M" /mnt/boot/firmware/cmdline.txt; then
  sed -i '$s/$/ cma=128M/' /mnt/boot/firmware/cmdline.txt
fi
