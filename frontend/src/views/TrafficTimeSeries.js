...
export default class TrafficTimeSeries extends Component {
    ...
    for (let target of targets) {
      ...
      for (let idx = 0; idx < traffic_series.length; idx++) {
        ...
        for (let ip of ips) {
          ...
          if (
            !traffic_series[idx][ip] ||
            !traffic_series[idx + 1] ||
            !traffic_series[idx + 1][ip]
          ) {
          } else {
            delta +=
              traffic_series[idx][ip][target] -
              traffic_series[idx + 1][ip][target]
          }
        }
      ...
      let ret = []
      let colors = chroma
        .scale(['0000ff', '00ff00', 'ffff00', 'ff7f00', '660000'])
        .mode('lch')
        .colors(ips.length)
      let colorIdx = 0
      for (let ip of ips) {
  ...
  legendClickHandler(e, legendItem, legend) {
    ...
    let ci = legend.chart
    let hiddenState
    ci.data.datasets[index].hidden = hiddenState =
  ...
  async componentDidMount() {
    ...
    this.templateTimeData.options.plugins.legend.onClick =
      this.legendClickHandler.bind(this)
  }
