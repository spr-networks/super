...
func TestPluginDHCPDoesNotTrustInterfaceAsLAN(t *testing.T) {
	...
	// Seed the stale trust produced by the old DHCP behavior and ensure that a
	// subsequent plugin lease removes it from both copies of the nftables set.
	...
	// Ordinary client DHCP still discovers and trusts its LAN interface.
